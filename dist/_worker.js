...
const MEDIA_EXT_REGEX = /\.(mp4|webm|ogg|mp3|wav|flac|aac|m4a|m4s|ts|flv|mkv|avi)$/i;
...
async function handleRequest(request, env) {
    ...
    if (target.hostname.startsWith('xn--') || target.hostname.includes('.xn--')) {
        return buildUIResponse("🚨 防钓鱼策略拦截", "Anti-Phishing Enforced", `目标域名 <b>${esc(target.hostname)}</b> 包含 Punycode 编码，属于典型的同形异义字（Homograph）钓鱼混淆特征，已被安全防御网关自动切断。`, 403);
    }
    ...
    if (clientReferer) {
        try {
            ...
            let refProxy = refUrl.searchParams.get('__up_proxy');
            if (refProxy) {
                finalReferer = safeDecode(refProxy).replace(/\/+$/, '') + refUrl.pathname + refUrl.search + refUrl.hash;
                reqOrigin = new URL(finalReferer).origin;
            } else {
    ...
    if (MEDIA_EXT_REGEX.test(target.pathname) || request.headers.has('range')) {
    ...
    const isDASH = !isHTML && (contentType.includes("dash+xml") || contentType.includes("dash.mpd") || target.pathname.toLowerCase().endsWith(".mpd"));
    
    const isMediaStream = !isM3U8 && !isDASH && (
        response.status === 206 || 
        ...
        MEDIA_EXT_REGEX.test(target.pathname)
    );
    ...
    if (isMediaStream) {
        ...
        let cc = newHeaders.get("Cache-Control");
        if (cc) {
            if (!cc.includes("no-transform")) newHeaders.set("Cache-Control", cc + ", no-transform");
        } else {
    ...
    const isXML = !isHTML && !isM3U8 && (contentType.includes("xml") || target.pathname.toLowerCase().endsWith(".xml") || isDASH);
    ...
    let outCc = newHeaders.get("Cache-Control");
    if (outCc) {
        if (!outCc.includes("no-transform")) newHeaders.set("Cache-Control", outCc + ", no-transform");
    } else {
        newHeaders.set("Cache-Control", "no-transform");
    }
...
async function rewriteTextResource(res, headers, proxyOrigin, target, isHTML) {
    ...
    let replacedText = text.replace(getOriginRegex(target.origin, isHTML), proxyOrigin + "/" + target.origin);
...
async function rewriteJSResponse(res, headers, proxyOrigin, target) {
    ...
    const isPlainText = ct.toLowerCase().includes("text/plain");
    
    if (!isMediaHoster && !isJson && !isPlainText && js.includes('location')) {
