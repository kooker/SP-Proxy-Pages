// dist/_worker.js (SP-Proxy-Pages Enterprise Edition v1.0.0)
"use strict";

const MAX_REWRITE_SIZE = 5 * 1024 * 1024; 
const SECRET_PREFIX = "";
const BOT_REGEX = /bot|spider|crawler|python|curl|wget|postman|scanner|shodan|masscan|nmap|zgrab|censys|projectdiscovery|nuclei|httpclient|java|ruby|go-http|phantomjs|headless/i;
const IGNORE_URL_REGEX = /^(data:|blob:|javascript:|mailto:|tel:|#)/i;
const PROTOCOL_REGEX = /^(https?):\/+/;
const CSS_URL_REGEX = /url\((['"]?)([^)'"\n]+)\1\)/gi; 
const UP_COOKIE_REGEX = /^__UP_(.+?)__(.*)$/;
const MEDIA_CDN_REGEX = /googlevideo\.com|youtube\.com|ytimg\.com|twitch\.tv|tiktokcdn\.com/i;
...
const ATTR_LIST =["src", "href", "action", "formaction", "data-url", "data-pjax-url", "data-src", "data-hovercard-url", "data-color-theme-request-url", "data-refresh-url", "data-contents-url", "data-fragment-url", "data-state-url", "data-data-url", "data-tree-url", "data-commit-hovercards-url", "data-async-url", "data-remote-url", "data-catalyst-url", "data-repository-hovercards-url", "data-ping-url", "data-target-url", "data-base-src", "data-base-url", "data-replace-remote-form-target"];
...
const DOM_SANDBOX_JS = `<script>!function(){if(typeof globalThis==='undefined')window.globalThis=window;if(globalThis.__UP_DOM_HOOKED)return;globalThis.__UP_DOM_HOOKED=!0;const P=__ProxyOrigin,T=__TargetOrigin;globalThis.__UP_TARGET=T;!function(){let rx=new RegExp('^'+P.replace(/[.*+?^$\\{\\}()|[\\]\\\\]/g,'\\\\$&')+'\\\\/+(https?(?::\\\\/\\\\/|%3A%2F%2F)[^/]+)','i'),m=location.href.match(rx);let isGH=/github\\.com/i.test(T)||(m&&/github\\.com/i.test(m[1]));if(m){let targetOrigin=m[1];try{if(targetOrigin.includes('%2Fhttps%3A')||targetOrigin.includes('%3A%2F%2F'))targetOrigin=decodeURIComponent(targetOrigin)}catch(e){}if(isGH){let cleanPath=location.href.substring(m[0].length)||'/';if(!cleanPath.startsWith('/'))cleanPath='/'+cleanPath;let u=new URL(cleanPath,location.origin);u.searchParams.set('__up_proxy',targetOrigin);history.replaceState(null,'',u.href)}}else if(isGH&&!location.search.includes('__up_proxy=')){try{let u=new URL(location.href);u.searchParams.set('__up_proxy',T);history.replaceState(null,'',u.href)}catch(e){}}}();function getTH(){try{let u=new URL(location.href);let rx=new RegExp('^'+P.replace(/[.*+?^$\\{\\}()|[\\]\\\\]/g,'\\\\$&')+'\\\\/+(https?(?::\\\\/\\\\/|%3A%2F%2F)[^/]+)','i');let m=location.href.match(rx);if(m){let tO=m[1];try{tO=decodeURIComponent(tO)}catch(e){}let cP=location.href.substring(m[0].length)||'/';if(!cP.startsWith('/'))cP='/'+cP;let cU=new URL(cP,location.origin);let res=tO+cU.pathname+cU.search+cU.hash;let nm;while((nm=res.match(/^(https?:\\/\\/[^\\/?#]+)\\/+(https?:\\/\\/.*)/i))){res=nm[2];}return res}u.searchParams.delete('__up_proxy');return T+(u.pathname==='/'&&!location.href.includes('//'+location.host+'/')?'/':u.pathname)+u.search+u.hash}catch(e){return T}}const TH=getTH();const AL2=['src','href','action','formaction','data-url','data-pjax-url','data-src','data-hovercard-url','data-color-theme-request-url','data-refresh-url','data-contents-url','data-fragment-url','data-state-url','data-data-url','data-tree-url','data-commit-hovercards-url','data-async-url','data-remote-url','data-catalyst-url','data-repository-hovercards-url','data-ping-url','data-target-url','data-base-src','data-base-url','data-replace-remote-form-target'];function _showPB(p){if(globalThis.__up_pb_shown)return;if(!p&&document.cookie.includes('__up_pb_ack=1'))return;if(!p)document.cookie='__up_pb_ack=1; max-age=2592000; path=/; samesite=lax';globalThis.__up_pb_shown=1;let f=()=>{let b=document.body||document.documentElement;if(!b)return;let d=document.createElement('div');d.innerHTML='⚠️ 防钓鱼预警：当前页面由第三方代理中转。非自部署，严禁在此输入任何账密凭据，谨防信息被拦截盗用！';d.style.cssText='position:fixed;top:0;left:0;width:100%;background:#ef4444;color:#fff;text-align:center;padding:12px;z-index:2147483647;font-size:14px;font-family:system-ui,sans-serif;box-shadow:0 4px 15px rgba(239,68,68,0.5);border-bottom:2px solid #b91c1c;transition:opacity 0.5s;pointer-events:none;';try{b.appendChild(d);setTimeout(()=>{d.style.opacity='0';setTimeout(()=>{try{d.remove()}catch(e){}globalThis.__up_pb_shown=0},500)},p?5000:8000)}catch(e){}};if(document.body)f();else globalThis.addEventListener('DOMContentLoaded',f)}globalThis.__up_crypto=function(){let p=null,sk=null,ek=null;async function g(){if(p)return p;let j=sessionStorage.getItem('__up_jwk');if(!j){let r=await fetch('/_up_pub_key');j=await r.text();sessionStorage.setItem('__up_jwk',j)}return p=await crypto.subtle.importKey("jwk",JSON.parse(j),{name:"RSA-OAEP",hash:"SHA-256"},!1,["encrypt"])}function b(f){let u=new Uint8Array(f),s='',c=8192;for(let i=0;i<u.length;i+=c)s+=String.fromCharCode.apply(null,u.subarray(i,i+c));return btoa(s)}return{async e(t){if(!sk){sk=await crypto.subtle.generateKey({name:"AES-GCM",length:256},!0,["encrypt"]);let k=await g(),r=await crypto.subtle.exportKey("raw",sk);ek=b(await crypto.subtle.encrypt({name:"RSA-OAEP"},k,r))}let i=crypto.getRandomValues(new Uint8Array(12)),d=await crypto.subtle.encrypt({name:"AES-GCM",iv:i},sk,new TextEncoder().encode(t));let ts=Date.now(),nonce=b(crypto.getRandomValues(new Uint8Array(12)));return btoa(JSON.stringify({r:ek,i:b(i),d:b(d),t:ts,n:nonce}))}}}();function toP(u){if(!u)return u;let s=String(u).trim();if(/^(data:|blob:|javascript:|mailto:|tel:|#)/i.test(s))return s;let pu;try{if(s.startsWith(P+'/http')){pu=s.substring(P.length+1);if(pu.includes('%3A%2F%2F'))try{pu=decodeURIComponent(pu)}catch(e){}}else if(s.startsWith(P+'/')&&!s.startsWith(P+'/http')){pu=T+s.substring(P.length);}else if(s===P){pu=T;}else{pu=new URL(s,getTH()).href;}}catch(e){return s}let m;while((m=pu.match(/^(https?:\\/\\/[^\\/?#]+)\\/+(https?:\\/\\/.*)/i))){pu=m[2];}if(pu.startsWith(P+'/')){pu=T+pu.substring(P.length);}else if(pu===P){pu=T;}return P+'/'+pu;}function unP(u){if(!u)return u;let s=String(u);if(s.startsWith(P+'/http')){try{let t=s.substring(P.length+1);if(t.includes('%2Fhttps%3A')||t.includes('%3A%2F%2F'))t=decodeURIComponent(t);let m;while((m=t.match(/^(https?:\\/\\/[^\\/?#]+)\\/+(https?:\\/\\/.*)/i))){t=m[2];}return t;}catch(e){}}return s}const _loc=new URL(getTH()),_up_proxy_obj=new Proxy(_loc,{get(t,p){let th=getTH();if(p===Symbol.toPrimitive||p==='toString'||p==='valueOf')return ()=>th;if(p==='href')return th;if(['pathname','origin','host','hostname','port','protocol','search','hash'].includes(p)){try{return new URL(th)[p]}catch(e){return ''}}if(p==='assign')return u=>location.assign(toP(u));if(p==='replace')return u=>location.replace(toP(u));if(p==='reload')return f=>location.reload(f);let v=location[p];return typeof v==='function'?v.bind(location):v;},set(t,p,v){if(['href','pathname','search','hash','protocol','host','hostname','port'].includes(p)){if(p==='href'){location.href=toP(v);return !0}try{let u=new URL(getTH());u[p]=v;location.href=toP(u.href)}catch(e){location[p]=toP(v)}return !0}location[p]=v;return !0}});Object.defineProperty(globalThis,'__up_loc',{get:()=>_up_proxy_obj,set:v=>{_up_proxy_obj.href=v;},configurable:!0});try{Object.defineProperty(document,'location',{get:()=>_up_proxy_obj,set:v=>{_up_proxy_obj.href=v;},configurable:!0});}catch(e){}try{function df(p){Object.defineProperty(Document.prototype,p,{get:()=>getTH(),configurable:!0});}df('URL');df('documentURI');df('baseURI');}catch(e){}try{Object.defineProperty(MessageEvent.prototype,'origin',{get:()=>T});}catch(e){}function hookWin(w){try{if(!w||w.__up_hw)return;w.__up_hw=!0;if(w.History){const _hp=w.History.prototype.pushState,_hr=w.History.prototype.replaceState;w.History.prototype.pushState=function(){let a=Array.from(arguments);if(a.length>2&&a[2]){try{let u=new URL(a[2],location.href);let isGH=/github\\.com/i.test(T);if(u.origin===T||u.origin===location.origin){if(isGH){u.searchParams.set('__up_proxy',T);a[2]=u.pathname+u.search+u.hash}else a[2]=toP(a[2])}else a[2]=toP(a[2])}catch(e){a[2]=toP(a[2])}}return _hp.apply(this,a)};w.History.prototype.replaceState=function(){let a=Array.from(arguments);if(a.length>2&&a[2]){try{let u=new URL(a[2],location.href);let isGH=/github\\.com/i.test(T);if(u.origin===T||u.origin===location.origin){if(isGH){u.searchParams.set('__up_proxy',T);a[2]=u.pathname+u.search+u.hash}else a[2]=toP(a[2])}else a[2]=toP(a[2])}catch(e){a[2]=toP(a[2])}}return _hr.apply(this,a)};}const _fRTC=function(){this.localDescription=null;this.remoteDescription=null;this.signalingState='closed';this.connectionState='closed';this.iceConnectionState='closed';this.iceGatheringState='complete'};_fRTC.prototype={close:function(){},createOffer:function(){return Promise.reject(new Error("WebRTC blocked"))},createAnswer:function(){return Promise.reject(new Error("WebRTC blocked"))},setLocalDescription:function(){return Promise.reject(new Error("WebRTC blocked"))},setRemoteDescription:function(){return Promise.reject(new Error("WebRTC blocked"))},addIceCandidate:function(){return Promise.reject(new Error("WebRTC blocked"))},addTrack:function(){},addTransceiver:function(){},addEventListener:function(){},removeEventListener:function(){},dispatchEvent:function(){return!0},getReceivers:function(){return[]},getSenders:function(){return[]},getTransceivers:function(){return[]},getConfiguration:function(){return{}},setConfiguration:function(){}};['RTCPeerConnection','webkitRTCPeerConnection','mozRTCPeerConnection'].forEach(n=>{if(w[n])w[n]=_fRTC});['RTCSessionDescription','RTCIceCandidate','RTCDataChannel'].forEach(n=>{if(w[n])w[n]=function(){}});if(w.navigator){if(w.navigator.mediaDevices&&w.navigator.mediaDevices.getUserMedia)w.navigator.mediaDevices.getUserMedia=function(){return Promise.reject(new Error("Media blocked"))};['getUserMedia','webkitGetUserMedia','mozGetUserMedia'].forEach(n=>{if(w.navigator[n])w.navigator[n]=function(c,s,e){if(e)e(new Error("Media blocked"))}});if(w.navigator.geolocation){w.navigator.geolocation.getCurrentPosition=function(s,e){if(e)e({code:1,message:"Geo blocked"})};w.navigator.geolocation.watchPosition=function(s,e){if(e)e({code:1,message:"Geo blocked"});return 0}}}}catch(e){}}hookWin(globalThis);const _Req=globalThis.Request;if(_Req)globalThis.Request=function(i,o){let r=i&&typeof i==='object'&&i.url,u=r?i.url:i;try{if(typeof u==='string')u=toP(u);}catch(e){}return new _Req(u,o||(r?i:undefined));};const _f=globalThis.fetch;globalThis.fetch=async function(i,o){let r=i&&typeof i==='object'&&i.url,u=r?i.url:i;let _ou=String(u);try{u=toP(u);}catch(e){}let initObj=o?Object.assign({},o):{};if(r){initObj.method=i.method;if(i.headers){let h=new Headers();if(i.headers instanceof Headers)i.headers.forEach((v,k)=>h.set(k,v));else if(Array.isArray(i.headers))i.headers.forEach(kv=>h.set(kv[0],kv[1]));else Object.keys(i.headers).forEach(k=>h.set(k,i.headers[k]));initObj.headers=h}if(i.method!=='GET'&&i.method!=='HEAD'&&!initObj.body&&!i.bodyUsed){try{initObj.body=await i.clone().blob();}catch(e){}}['credentials','cache','redirect','referrer','integrity'].forEach(k=>{if(i[k]!==undefined)initObj[k]=i[k];});if(i.mode==='navigate')initObj.mode='cors';else initObj.mode=i.mode;if(i.signal)initObj.signal=i.signal;}if(o){if(o.headers){let merged=new Headers(initObj.headers||{});new Headers(o.headers).forEach((v,k)=>merged.set(k,v));initObj.headers=merged;}if(o.mode==='navigate')initObj.mode='cors';}let m=initObj.method||'GET',b=initObj.body;let isLg=/login|sign-?in|auth/i.test(_ou);if(isLg&&b&&m&&['POST','PUT','PATCH'].includes(m.toUpperCase())){let strBody=null;if(typeof b==='string')strBody=b;else if(b instanceof URLSearchParams)strBody=b.toString();else if(b instanceof FormData){let hasFile=!1;if(b.values){for(let val of b.values()){if(val instanceof File||val instanceof Blob)hasFile=!0;}}if(!hasFile)strBody=new URLSearchParams(b).toString();}if(strBody!==null&&strBody.length<500000){try{let c=await globalThis.__up_crypto.e(strBody);if(!initObj.headers)initObj.headers=new Headers();let hd=new Headers(initObj.headers);hd.set('X-Up-Enc','1');if(!hd.has('content-type')&&!hd.has('Content-Type'))hd.set('Content-Type','application/x-www-form-urlencoded');initObj.body='__UP_ENC__'+c;initObj.headers=hd;return _f.call(globalThis,u,initObj);}catch(e){}}}return _f.call(globalThis,u,initObj);};const _o=XMLHttpRequest.prototype.open;XMLHttpRequest.prototype.open=function(m,u,...r){this.__up_u=String(u);try{u=toP(u);}catch(e){}return _o.call(this,m,u,...r);};const _s=XMLHttpRequest.prototype.send;XMLHttpRequest.prototype.send=function(b){let _ou=this.__up_u||'';let isLg=/login|sign-?in|auth/i.test(_ou);if(isLg&&b){let strBody=null;if(typeof b==='string')strBody=b;else if(b instanceof URLSearchParams)strBody=b.toString();else if(b instanceof FormData){let hasFile=!1;if(b.values){for(let val of b.values()){if(val instanceof File||val instanceof Blob)hasFile=!0;}}if(!hasFile)strBody=new URLSearchParams(b).toString();}if(strBody!==null&&strBody.length<500000){let t=this;globalThis.__up_crypto.e(strBody).then(c=>{t.setRequestHeader('X-Up-Enc','1');if(b instanceof FormData||b instanceof URLSearchParams)t.setRequestHeader('Content-Type','application/x-www-form-urlencoded');_s.call(t,'__UP_ENC__'+c)}).catch(()=>{_s.call(t,b)});return}} _s.call(this,b)};const _wo=globalThis.open;if(_wo)globalThis.open=function(u,n,f){try{u=toP(u);}catch(e){}return _wo.call(this,u,n,f);};if(navigator.sendBeacon){const _sb=navigator.sendBeacon;navigator.sendBeacon=function(u,d){try{u=toP(u);}catch(e){}return _sb.call(this,u,d);};}const _Wk=globalThis.Worker;if(_Wk)globalThis.Worker=function(s,o){try{s=toP(s);}catch(e){}return new _Wk(s,o);};const _SWk=globalThis.SharedWorker;if(_SWk)globalThis.SharedWorker=function(s,o){try{s=toP(s);}catch(e){}return new _SWk(s,o);};const _ES=globalThis.EventSource;if(_ES)globalThis.EventSource=function(s,o){try{s=toP(s);}catch(e){}return new _ES(s,o);};const _W=globalThis.WebSocket;if(_W)globalThis.WebSocket=function(u,p){try{let w=new URL(String(u),getTH());if(w.protocol==='ws:'||w.protocol==='wss:'){let t=(w.protocol==='ws:'?'http:':'https:')+'//'+w.host+w.pathname+w.search;u=P.replace('http','ws')+'/'+t;}}catch(e){}return p?new _W(u,p):new _W(u);};const _pm=Window.prototype.postMessage;Window.prototype.postMessage=function(d,o,t){if(typeof o==='string'&&o!=='/'&&o!=='*')o='*';if(arguments.length<3)return _pm.call(this,d,o);return _pm.call(this,d,o,t);};const _fs=HTMLFormElement.prototype.submit;async function encF(f,btn){let a=String(f.action||getTH());let isLg=/login|sign-?in|auth/i.test(a);if(isLg&&f.method&&f.method.toUpperCase()==='POST'&&!f.hasAttribute('data-up-enc')&&f.enctype!=='multipart/form-data'&&!f.querySelector('input[type="file"]')){try{let fd=new FormData(f);if(btn&&btn.name)fd.append(btn.name,btn.value||'');else{let sBtn=f.querySelector('button[type="submit"][name], input[type="submit"][name]');if(sBtn&&sBtn.name)fd.append(sBtn.name,sBtn.value||'');}if(isLg&&!fd.has('loginsubmit')){fd.append('loginsubmit','yes');}if(!fd.has('formhash')){let fh=document.querySelector('input[name="formhash"]');if(fh)fd.append('formhash',fh.value);}let d=new URLSearchParams(fd).toString(),c=await globalThis.__up_crypto.e(d);f.setAttribute('data-up-enc','1');let i=document.createElement('input');i.type='hidden';i.name='__UP_ENC__';i.value=c;i.className='__up_e_i';f.appendChild(i);f.action=toP(f.action||getTH());const nx=document.createElement('input');nx.type='hidden';nx.name='__UP_ENCH__';nx.value='1';f.appendChild(nx);_fs.call(f);setTimeout(()=>{f.removeAttribute('data-up-enc');f.querySelectorAll('.__up_e_i, input[name="__UP_ENCH__"]').forEach(el=>el.remove());},100);return !0;}catch(x){}}return !1;}HTMLFormElement.prototype.submit=function(){let f=this;encF(f,null).then(r=>{if(!r)_fs.call(f)});};globalThis.addEventListener('submit',e=>{if(e.defaultPrevented)return;let f=e.target;if(f.method&&f.method.toUpperCase()==='POST'&&!f.hasAttribute('data-up-enc')&&f.enctype!=='multipart/form-data'&&!f.querySelector('input[type="file"]')){e.preventDefault();encF(f,e.submitter).then(r=>{if(!r)_fs.call(f)});}},!1);let _pwHk=e=>{if(e.target&&e.target.tagName==='INPUT'&&e.target.type==='password'){_showPB(1);}};globalThis.addEventListener('focusin',_pwHk,!1);globalThis.addEventListener('input',_pwHk,!1);_showPB(0);function hk(C,N,S){if(!C||!C.prototype)return;const d=Object.getOwnPropertyDescriptor(C.prototype,N);if(d&&d.set){const s_fn=d.set,g_fn=d.get;try{Object.defineProperty(C.prototype,N,{set:function(v){if(S)return;try{if(!this.hasAttribute('data-up-orig-'+N.toLowerCase()))this.setAttribute('data-up-orig-'+N.toLowerCase(),v);v=toP(v);}catch(e){}s_fn.call(this,v);},get:function(){let v=g_fn.call(this);return(N==='href'||N==='src'||N==='action')?unP(v):v;}});}catch(e){}}}try{hk(HTMLScriptElement,'src',0);hk(HTMLLinkElement,'href',0);hk(HTMLImageElement,'src',0);hk(HTMLIFrameElement,'src',0);hk(HTMLAnchorElement,'href',0);hk(HTMLFormElement,'action',0);hk(HTMLButtonElement,'formAction',0);hk(HTMLVideoElement,'src',0);hk(HTMLAudioElement,'src',0);hk(HTMLMediaElement,'src',0);hk(HTMLSourceElement,'src',0);hk(HTMLTrackElement,'src',0);hk(HTMLScriptElement,'integrity',1);hk(HTMLLinkElement,'integrity',1);hk(HTMLScriptElement,'nonce',1);hk(HTMLElement,'nonce',1);}catch(e){}['pathname','search','hash','host','hostname','port','protocol','origin'].forEach(p=>{const d=Object.getOwnPropertyDescriptor(HTMLAnchorElement.prototype,p);if(d&&d.get){const s_fn=d.set,g_fn=d.get;try{Object.defineProperty(HTMLAnchorElement.prototype,p,{get:function(){let h=unP(this.href);try{return new URL(h)[p];}catch(e){return g_fn.call(this);}},set:function(v){if(s_fn){let h=unP(this.href);try{let u=new URL(h);u[p]=v;this.href=u.href;}catch(e){s_fn.call(this,v);}}}});}catch(e){}}});const _ga=Element.prototype.getAttribute;Element.prototype.getAttribute=function(n){let v=_ga.call(this,n);if(v&&typeof v==='string'&&AL2.includes(n.toLowerCase())){let orig=_ga.call(this,'data-up-orig-'+n.toLowerCase());if(orig)return orig;return unP(v);}return v;};const _sa=Element.prototype.setAttribute;Element.prototype.setAttribute=function(n,v){const l=String(n).toLowerCase();if(['integrity','crossorigin','nonce'].includes(l))return;if(AL2.includes(l)&&v){try{if(!this.hasAttribute('data-up-orig-'+l))_sa.call(this,'data-up-orig-'+l,v);v=toP(v);}catch(e){}}return _sa.call(this,n,v);};if(navigator.serviceWorker&&navigator.serviceWorker.register){navigator.serviceWorker.register('/sw.js',{scope:'/'}).catch(()=>{});navigator.serviceWorker.register=()=>Promise.reject(new Error("SW disabled by proxy."));}try{const _cd=Object.getOwnPropertyDescriptor(Document.prototype,'cookie')||Object.getOwnPropertyDescriptor(HTMLDocument.prototype,'cookie');if(_cd&&_cd.configurable){Object.defineProperty(document,'cookie',{get:function(){let th_h=new URL(getTH()).hostname,a=_cd.get.call(this)||"",ps=a.split(';'),r=[];for(let i=0;i<ps.length;i++){let p=ps[i].trim(),eq=p.indexOf('='),n=eq===-1?p:p.substring(0,eq),v=eq===-1?'':p.substring(eq),m=n.match(/^__UP_(.+?)__(.*)$/);if(m){let d=m[1],cn=m[2];if(th_h===d||th_h.endsWith('.'+d))r.push(cn+v);}}return r.join('; ');},set:function(vl){let th_h=new URL(getTH()).hostname,ps=vl.toString().split(';'),nv=ps[0],eq=nv.indexOf('='),n=eq===-1?nv.trim():nv.substring(0,eq).trim(),cv=eq===-1?'':nv.substring(eq),d=th_h,o=[];for(let i=1;i<ps.length;i++){let p=ps[i].trim(),pl=p.toLowerCase();if(pl.startsWith('domain='))d=p.substring(7).trim().replace(/^\\./,'');else if(!pl.startsWith('path=')&&!pl.startsWith('samesite')&&!pl.startsWith('secure'))o.push(p);}let ut='__UP_'+d+'__'+n+cv+'; Path=/; SameSite=None; Secure';if(o.length)ut+='; '+o.join('; ');_cd.set.call(this,ut);}});}Object.defineProperty(document,'domain',{get:()=>new URL(getTH()).hostname,set:function(){}});}catch(e){}try{const _resUrl=Object.getOwnPropertyDescriptor(Response.prototype,'url');if(_resUrl&&_resUrl.get){Object.defineProperty(Response.prototype,'url',{get:function(){return unP(_resUrl.get.call(this))},configurable:!0})}const _reqUrl=Object.getOwnPropertyDescriptor(Request.prototype,'url');if(_reqUrl&&_reqUrl.get){Object.defineProperty(Request.prototype,'url',{get:function(){return unP(_reqUrl.get.call(this))},configurable:!0})}const _xhrUrl=Object.getOwnPropertyDescriptor(XMLHttpRequest.prototype,'responseURL');if(_xhrUrl&&_xhrUrl.get){Object.defineProperty(XMLHttpRequest.prototype,'responseURL',{get:function(){return unP(_xhrUrl.get.call(this))},configurable:!0})}}catch(e){}function arYT(){try{let t=new URL(getTH());if(!globalThis.__up_yt_locked){globalThis.__up_yt_locked=!0;try{let _vp=HTMLMediaElement.prototype.play;HTMLMediaElement.prototype.play=function(){let u=new URL(getTH());if(u.hostname==='m.youtube.com'&&u.pathname.startsWith('/watch')){try{this.muted=!0;this.volume=0}catch(e){}}return _vp.call(this)};let oV=Object.getOwnPropertyDescriptor(HTMLMediaElement.prototype,'volume');if(oV){Object.defineProperty(HTMLMediaElement.prototype,'volume',{get:function(){let u=new URL(getTH());return(u.hostname==='m.youtube.com'&&u.pathname.startsWith('/watch'))?0:oV.get.call(this)},set:function(v){let u=new URL(getTH());if(!(u.hostname==='m.youtube.com'&&u.pathname.startsWith('/watch')))oV.set.call(this,v)},configurable:!0})}let oM=Object.getOwnPropertyDescriptor(HTMLMediaElement.prototype,'muted');if(oM){Object.defineProperty(HTMLMediaElement.prototype,'muted',{get:function(){let u=new URL(getTH());return(u.hostname==='m.youtube.com'&&u.pathname.startsWith('/watch'))?!0:oM.get.call(this)},set:function(v){let u=new URL(getTH());if(!(u.hostname==='m.youtube.com'&&u.pathname.startsWith('/watch')))oM.set.call(this,v)},configurable:!0})}}catch(e){}}if(t.hostname==='m.youtube.com'){if(t.pathname.startsWith('/watch')){let v=t.searchParams.get('v');if(v){let p=document.querySelector('.html5-video-player, #player, #movie_player, .player-container');if(p){let vds=p.querySelectorAll('video');for(let i=0;i<vds.length;i++){let n=vds[i];n.muted=!0;n.volume=0;n.pause();try{if(n.hasAttribute('src')||n.src){n.removeAttribute('src');n.src='';n.load()}if(n.srcObject)n.srcObject=null}catch(err){}n.style.opacity='0';n.style.pointerEvents='none'}let ols=p.querySelectorAll('.ytp-chrome-top, .ytp-chrome-bottom, .ytp-spinner, .ytp-error, .ytp-ad-module, .html5-video-info-panel, .ytp-gradient-top, .ytp-gradient-bottom, ytm-custom-control, .player-controls');for(let k=0;k<ols.length;k++){ols[k].style.opacity='0';ols[k].style.pointerEvents='none'}let e=document.getElementById('up-yt-iframe-container');if(e){if(e.dataset.vid!==v){e.querySelector('iframe').src=toP('https://www.youtube.com/embed/'+v+'?autoplay=1&playsinline=1');e.dataset.vid=v}}else{let c=document.createElement('div');c.id='up-yt-iframe-container';c.dataset.vid=v;c.style.cssText="position:absolute;top:0;left:0;width:100%;height:100%;z-index:2147483647;background:#000;";let f=document.createElement('iframe');f.width="100%";f.height="100%";f.src=toP('https://www.youtube.com/embed/'+v+'?autoplay=1&playsinline=1');f.title="YouTube proxy player";f.setAttribute("frameborder","0");f.setAttribute("allow","accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share");f.setAttribute("sandbox","allow-same-origin allow-scripts allow-forms allow-popups allow-popups-to-escape-sandbox allow-presentation");f.setAttribute("referrerpolicy","strict-origin-when-cross-origin");f.setAttribute("allowfullscreen","");f.style.cssText="width:100%;height:100%;border:none;";c.appendChild(f);p.appendChild(c)}}}}else{let e=document.getElementById('up-yt-iframe-container');if(e)e.remove();let vds=document.querySelectorAll('video');for(let i=0;i<vds.length;i++){let n=vds[i];if(n.style.opacity==='0'){n.style.opacity='1';n.style.pointerEvents='auto'}}let ols=document.querySelectorAll('.ytp-chrome-top, .ytp-chrome-bottom, .ytp-spinner, .ytp-error, .ytp-ad-module, .html5-video-info-panel, .ytp-gradient-top, .ytp-gradient-bottom, ytm-custom-control, .player-controls');for(let k=0;k<ols.length;k++){if(ols[k].style.opacity==='0'){ols[k].style.opacity='1';ols[k].style.pointerEvents='auto'}}}}}catch(x){}try{const r=new RegExp('(?:youtube\\\\.com\\\\/(?:[^\\\\/]+\\\\/.+\\\\/|(?:v|e(?:mbed)?)\\\\/|.*[?&]v=)|youtu\\\\.be\\\\/)([^"&?\\\\/\\\\s]{11})','i');let l=document.querySelectorAll('a:not([data-yt-replaced])');for(let i=0;i<l.length;i++){let a=l[i];a.setAttribute('data-yt-replaced','1');let h=a.getAttribute('data-up-orig-href')||a.href||'';let m=h.match(r);if(m&&m[1]){let txt=a.textContent.trim();if(txt.includes('youtube.com')||txt.includes('youtu.be')){let c=document.createElement('div');c.style.cssText="position:relative;width:100%;padding-bottom:56.25%;height:0;overflow:hidden;background:#000;border-radius:8px;margin:10px 0;";let f=document.createElement('iframe');f.src=toP('https://www.youtube.com/embed/'+m[1]+'?autoplay=0&playsinline=1');f.title="YouTube proxy player";f.setAttribute("frameborder","0");f.setAttribute("allow","accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share");f.setAttribute("sandbox","allow-same-origin allow-scripts allow-forms allow-popups allow-popups-to-escape-sandbox allow-presentation");f.setAttribute("referrerpolicy","strict-origin-when-cross-origin");f.setAttribute("allowfullscreen","");f.style.cssText="position:absolute;top:0;left:0;width:100%;height:100%;border:none;";c.appendChild(f);a.parentNode.replaceChild(c,a)}}}}catch(x){}}setInterval(arYT,1000);const _pN=(n)=>{if(n.removeAttribute){n.removeAttribute('integrity');n.removeAttribute('crossorigin');n.removeAttribute('nonce');}if(n.nodeName==='SCRIPT'&&n.textContent){if(!/youtube\\.com|ytimg\\.com|googlevideo\\.com/i.test(T)){let t=n.textContent;if(t.includes('location')){let r=t.replace(/\\b(window|document|top|parent|self)\\.location\\b/g,"globalThis.__up_loc");if(t!==r)try{n.textContent=r}catch(e){}}}}if(n.querySelectorAll){const els=n.querySelectorAll('[integrity],[crossorigin],[nonce]');for(let k=0;k<els.length;k++){els[k].removeAttribute('integrity');els[k].removeAttribute('crossorigin');els[k].removeAttribute('nonce');}const sl=AL2.map(a=>'['+a+']').join(',');const urls=n.querySelectorAll(sl);for(let k=0;k<urls.length;k++){let el=urls[k];AL2.forEach(attr=>{if(el.hasAttribute(attr)){let v=el.getAttribute(attr);if(v&&!v.startsWith(P+'/http')&&!/^(data:|blob:|javascript:|mailto:|tel:|#)/i.test(v)){try{if(!el.hasAttribute('data-up-orig-'+attr))el.setAttribute('data-up-orig-'+attr,v);el.setAttribute(attr,toP(v));}catch(e){}}}});}}AL2.forEach(attr=>{if(n.hasAttribute&&n.hasAttribute(attr)){let v=n.getAttribute(attr);if(v&&!v.startsWith(P+'/http')&&!/^(data:|blob:|javascript:|mailto:|tel:|#)/i.test(v)){try{if(!n.hasAttribute('data-up-orig-'+attr))n.setAttribute('data-up-orig-'+attr,v);n.setAttribute(attr,toP(v));}catch(e){}}}});};new MutationObserver(ms=>{for(let i=0;i<ms.length;i++){for(let j=0;j<ms[i].addedNodes.length;j++){let n=ms[i].addedNodes[j];if(n.nodeType===1)_pN(n);}}}).observe(document.documentElement,{childList:!0,subtree:!0});try{['innerHTML','outerHTML'].forEach(p=>{const d=Object.getOwnPropertyDescriptor(Element.prototype,p);if(d&&d.set){const _s=d.set;Object.defineProperty(Element.prototype,p,{set:function(v){if(typeof v==='string')v=v.replace(/\\s+(?:integrity|crossorigin|nonce)\\s*=\\s*(?:'[^']*'|"[^"]*")/gi,'');_s.call(this,v);try{_pN(this)}catch(e){}},get:d.get,configurable:!0});}});}catch(e){}try{const _cW=Object.getOwnPropertyDescriptor(HTMLIFrameElement.prototype,'contentWindow');if(_cW&&_cW.get)Object.defineProperty(HTMLIFrameElement.prototype,'contentWindow',{get:function(){let w=_cW.get.call(this);hookWin(w);if(w&&!w.__up_fw){w.__up_fw=1;try{w.fetch=globalThis.fetch;w.XMLHttpRequest=globalThis.XMLHttpRequest;w.Request=globalThis.Request;}catch(e){}}return w},configurable:!0});const _cD=Object.getOwnPropertyDescriptor(HTMLIFrameElement.prototype,'contentDocument');if(_cD&&_cD.get)Object.defineProperty(HTMLIFrameElement.prototype,'contentDocument',{get:function(){let d=_cD.get.call(this);if(d&&d.defaultView)hookWin(d.defaultView);return d},configurable:!0});}catch(e){}}();</script>`;
...
function getEnvHash(env) {
    return `${env.BLOCK_RULES || ''}|${env.ALLOW_RULES || ''}|${env.CRYPTO_SALT || ''}|${env.AUTH_SECRET || ''}`;
}
...
function compileRulesOnce(env) {
    ...
    if (currentHash === envHashCache) return;
    envHashCache = currentHash;
...
export default {
    async fetch(request, env) {
        try { return await handleRequest(request, env); } 
            ...
            return buildUIResponse("💥 网关致命异常", "System Error", "系统处理请求时遭遇未预期的底层异常。出于安全策略保护，异常堆栈与内部详情已被永久静默与隔离锁死。", 502); 
        }
    }
};
...
async function handleRequest(request, env) {
    ...
    if (authSecret) {
        ...
        const safeSecret = encodeURIComponent(authSecret).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        const isAuthed = new RegExp(`(?:^|;\\s*)__UP_AUTH__=${safeSecret}(?:;|$)`).test(cookieStr);
    ...
    let originalClean = clean;
    let nestedMatch;
    while ((nestedMatch = clean.match(/^(https?:\/\/[^\/?#]+)\/+(https?:\/\/.*)/i))) {
        clean = nestedMatch[2];
    }
    
    if (originalClean !== clean) {
        let redirectUrl = url.origin + (SECRET_PREFIX ? SECRET_PREFIX + "/" : "/") + clean;
    ...
    if (!isPrefixed) {
        if (!clean && !url.searchParams.has('__up_proxy')) return buildHomepage(url);
    ...
    let target;
    if (isPrefixed) {
        try { target = new URL(clean); } catch { return buildUIResponse("❌ 无效的 URL", "Invalid URL", "目标地址格式错误或解析失败，无法构建访问。", 400); }
    } else {
        let redirectTarget = url.searchParams.get('__up_proxy');
        if (redirectTarget) redirectTarget = safeDecode(redirectTarget);
        ...
        if (!redirectTarget) {
            ...
            if (ref) {
                try {
                    ...
                    let refProxy = refUrl.searchParams.get('__up_proxy');
                    if (refProxy) redirectTarget = safeDecode(refProxy);
                    else {
                        let refPath = refUrl.pathname.slice(1).replace(SECRET_PREFIX ? SECRET_PREFIX.slice(1) : "", "").replace(/^\/+/, "");
                        if (refPath.includes('%2Fhttps%3A')) refPath = safeDecode(refPath);
                        let em = refPath.match(/^(https?:\/\/[^\/]+)\/+(https?:\/\/.*)/i);
                        if (em) redirectTarget = new URL(em[2]).origin;
                    }
                } catch(e) {}
            }
        }
        ...
        if (redirectTarget) {
            try {
                target = new URL(redirectTarget.replace(/\/+$/, '') + "/" + clean);
                target.searchParams.delete('__up_proxy');
            } catch { return buildUIResponse("❌ 无效的 URL", "Invalid URL", "无法构建目标地址。", 400); }
        } else {
            return buildUIResponse("❓ 上下文丢失", "Context Lost", "无法正确解析目标地址，Worker 裸请求溯源失败。", 400);
        }
    }
    ...
    if ((target.protocol === 'http:' && target.port === '443') || (target.protocol === 'https:' && target.port === '80')) {
        return buildUIResponse("🔒 协议握手异常", "Protocol Mismatch", "检测到目标存在协议与端口混淆 (Protocol Confusion) 风险，为防止降级攻击与 TLS 剥离，已强制熔断。", 403);
    }

    if (env.STRICT_SSL_MODE === '1' && target.protocol === 'http:' && !PRIVATE_IP_REGEX.test(target.hostname)) {
    ...
    let ipHash = 0;
    const len = realIp.length;
    for (let i = 0; i < len; i++) {
        ipHash = (Math.imul(31, ipHash) + realIp.charCodeAt(i)) | 0;
    }
    ...
    const isMediaCDN = MEDIA_CDN_REGEX.test(target.hostname);
    if (!isMediaCDN) {
    ...
    if (reqCookies) {
        ...
        for (let pair of reqCookies.split(';')) {
            ...
            if (name === '__UP_AUTH__' || name === '__UP_LAST_TARGET__' || name === '__up_pb_ack') continue;
    ...
    if (clientReferer) {
        try {
            ...
            let refProxy = refUrl.searchParams.get('__up_proxy');
            if (refProxy) {
                let tempUrl = new URL(safeDecode(refProxy).replace(/\/+$/, '') + refUrl.pathname + refUrl.search + refUrl.hash);
                tempUrl.searchParams.delete('__up_proxy');
                finalReferer = tempUrl.href;
                reqOrigin = tempUrl.origin;
            } else {
                let refPath = refUrl.pathname.slice(1).replace(SECRET_PREFIX ? SECRET_PREFIX.slice(1) : "", "").replace(/^\/+/, "").replace(PROTOCOL_REGEX, "$1://");
                if (refPath.includes('%2Fhttps%3A')) refPath = safeDecode(refPath);
                let em = refPath.match(/^(https?:\/\/[^\/]+)\/+(https?:\/\/.*)/i);
                if (em) refPath = em[2];
    ...
    if (headers.has('x-pjax-url')) {
        let pjax = headers.get('x-pjax-url');
        try {
            if (pjax.startsWith(baseProxyOrigin)) {
                let parsed = new URL(pjax);
                let pjaxProxy = parsed.searchParams.get('__up_proxy');
                if (pjaxProxy) {
                    let tempUrl = new URL(safeDecode(pjaxProxy).replace(/\/+$/, '') + parsed.pathname + parsed.search + parsed.hash);
                    tempUrl.searchParams.delete('__up_proxy');
                    headers.set('x-pjax-url', tempUrl.href);
                } else {
                    let pjaxPath = parsed.pathname.slice(1).replace(SECRET_PREFIX ? SECRET_PREFIX.slice(1) : "", "").replace(/^\/+/, "").replace(PROTOCOL_REGEX, "$1://");
                    if (pjaxPath.includes('%2Fhttps%3A')) pjaxPath = safeDecode(pjaxPath);
                    let em = pjaxPath.match(/^(https?:\/\/[^\/]+)\/+(https?:\/\/.*)/i);
                    if (em) pjaxPath = em[2];
                    if (pjaxPath.startsWith("http")) headers.set('x-pjax-url', pjaxPath);
                }
            } else if (pjax.startsWith('/')) {
                let tempUrl = new URL(pjax, target.origin);
                if (tempUrl.searchParams.has('__up_proxy')) {
                    tempUrl.searchParams.delete('__up_proxy');
                    headers.set('x-pjax-url', tempUrl.pathname + tempUrl.search + tempUrl.hash);
                }
            }
        } catch (e) {}
    }

    const fetchOpts = { method: request.method, headers, redirect: "manual", signal: request.signal };
                ...
                if (encPayload) {
                    ...
                    try {
                        ...
                        if (parsed.t) {
                            ...
                            const maxAge = parseInt(env.REPLAY_WINDOW_MS) || 300000;
    ...
    let response;
    const canRetry = !request.body || typeof fetchOpts.body === 'string' || fetchOpts.body instanceof URLSearchParams;
    
    try { 
        response = await fetch(target, fetchOpts); 
        
        let attempts = 0;
        while ((response.status === 429 || response.status === 503) && attempts < 3 && canRetry) {
            let delayMs = 1500 * Math.pow(1.5, attempts) + Math.floor(Math.random() * 500);
            try {
                const ra = response.headers.get("Retry-After");
                if (ra) {
                    const raSecs = parseInt(ra, 10);
                    if (!isNaN(raSecs) && raSecs > 0 && raSecs <= 10) delayMs = raSecs * 1000;
                    else {
                        const raDate = new Date(ra).getTime();
                        if (!isNaN(raDate) && raDate > Date.now() && (raDate - Date.now()) <= 10000) delayMs = raDate - Date.now();
                    }
                }
            } catch (e) {}
        ...
        const errStr = String(err && err.cause ? err.cause : err).toLowerCase();
        if (errStr.includes("ssl") || errStr.includes("tls") || errStr.includes("cert") || errStr.includes("handshake") || errStr.includes("protocol error")) {
            return buildUIResponse("🔒 上游证书安全阻断", "Strict SSL Enforced", `目标主机 (<b>${esc(target.hostname)}</b>) 的 SSL/TLS 证书不可信、已过期或发生 SNI 握手异常。<br><br>基于军工级 Strict SSL 强校验策略，为防范中间人劫持 (MITM) 与降级攻击，网关已主动切断不安全链路。`, 502);
        }

        if (err.name !== 'AbortError' && canRetry && (request.method === "GET" || request.method === "HEAD")) {
            try {
                response = await fetch(target, fetchOpts);
            } catch (err2) {
                return buildUIResponse("⌛ 网关请求失败", "Upstream Unavailable", "上游连接故障。边缘节点原路重试失败，请检查网络链路。", 504); 
            }
        } else {
            return buildUIResponse("⌛ 网关请求失败", "Upstream Unavailable", "上游目标服务器连接被拒绝或未能及时响应。这通常由于目标节点网络故障或屏蔽所致，请稍后重试。", 504); 
        }
    }
    ...
    newHeaders.set("Server", "nginx");
    newHeaders.set("X-Powered-By", "PHP/8.1.0");
    ...
    const isRawDomain = /raw\.githubusercontent\.com|gist\.githubusercontent\.com|pastebin\.com/i.test(target.hostname);
    const isTextPlain = contentType.includes("text/plain");

    const isXML = !isHTML && !isM3U8 && !isRawDomain && !isTextPlain && (contentType.includes("xml") || target.pathname.toLowerCase().endsWith(".xml"));
    const isJS = !isHTML && !isM3U8 && !isRawDomain && !isTextPlain && (contentType.includes("javascript") || contentType.includes("json") || target.pathname.toLowerCase().endsWith(".js"));
    ...
    newHeaders.set("Permissions-Policy", "camera=(), microphone=(), geolocation=(), display-capture=()");
    ...
    ['x-pjax-url', 'turbo-location', 'pjax-url'].forEach(headerName => {
        const val = newHeaders.get(headerName);
        if (val) {
            try {
                const absUrl = new URL(val, target).href;
                if (!absUrl.startsWith(baseProxyOrigin)) newHeaders.set(headerName, baseProxyOrigin + "/" + absUrl);
            } catch {}
        }
    });
    ...
    if (typeof newHeaders.getSetCookie === 'function') {
        ...
        if (cookies.length) {
            ...
            for (let cookie of cookies) {
                ...
                if (name === '__UP_LAST_TARGET__' || name === '__up_pb_ack') { newHeaders.append("set-cookie", cookie); continue; }

                let origDomain = target.hostname.replace(/[^a-zA-Z0-9.-]/g, ''), extraParts =[];
                for (let i = 1; i < parts.length; i++) {
                    let p = parts[i].trim(), pl = p.toLowerCase();
                    if (pl.startsWith('domain=')) origDomain = p.substring(7).trim().replace(/^\./, '').replace(/[^a-zA-Z0-9.-]/g, '');
...
async function getDecodedText(res, ct) {
    ...
    if (match) {
        ...
        let headStr = '';
        for (let i = 0; i < chunk.length; i++) headStr += String.fromCharCode(chunk[i]);
        
        let metaMatch = headStr.match(/<meta[^>]*?charset\s*=\s*['"]?([a-zA-Z0-9\-_]+)/i);
        if (!metaMatch) metaMatch = headStr.match(/<meta[^>]*?content\s*=\s*['"][^"']*?charset\s*=\s*([a-zA-Z0-9\-_]+)/i);
        if (!metaMatch) metaMatch = headStr.match(/<\?xml[^>]*?encoding\s*=\s*['"]([a-zA-Z0-9\-_]+)/i);
        if (!metaMatch) metaMatch = headStr.match(/@charset\s*['"]([a-zA-Z0-9\-_]+)/i);
...
function rewriteOriginsInText(text, proxyOrigin, targetOrigin) {
    if (!text || !targetOrigin) return text;
    const originEscaped = targetOrigin.replace(/\//g, '(?:/|\\\\/)');
    const originRegex = new RegExp(`(['"])(${originEscaped})(?![a-zA-Z0-9.-])`, 'gi');
    return text.replace(originRegex, (m, q, p1) => {
        let prefix = proxyOrigin;
        if (p1.includes('\\/')) prefix = proxyOrigin.replace(/\//g, '\\/');
        return q + prefix + (p1.includes('\\/') ? '\\/' : '/') + p1;
    });
}
...
async function rewriteTextResource(res, headers, proxyOrigin, target, isHTML) {
    ...
    let replacedText = rewriteOriginsInText(text, proxyOrigin, target.origin);
    replacedText = replacedText.replace(getOriginRegex(target.origin, isHTML), proxyOrigin + "/" + target.origin);
    if (!isHTML) replacedText = replacedText.replace(/<\?xml([^>]*?)encoding\s*=\s*['"][a-zA-Z0-9\-_]+['"]/gi, '<?xml$1encoding="utf-8"');
    
    return new Response(replacedText, { status: res.status, headers });
}
...
async function rewriteCSSResponse(res, headers, proxyOrigin, target) {
    ...
    css = css.replace(/@charset\s*['"][a-zA-Z0-9\-_]+['"];?/gi, '@charset "utf-8";');
...
async function rewriteJSResponse(res, headers, proxyOrigin, target) {
    ...
    const isMediaHoster = MEDIA_CDN_REGEX.test(target.hostname);
    const isJSON = ct.toLowerCase().includes("json") || target.pathname.toLowerCase().endsWith(".json");
    
    if (!isMediaHoster) {
        js = rewriteOriginsInText(js, proxyOrigin, target.origin);
        
        if (!isJSON) {
            if (js.includes('location')) {
                js = js.replace(/\b(window|document|top|parent|self)\.location\b/g, "globalThis.__up_loc");
                js = js.replace(/(?<!\w|\.)location\.(pathname|href|origin|host|hostname|port|protocol|assign|replace|search|hash|reload)\b/g, "globalThis.__up_loc.$1").replace(/(?<!\w|\.)location(\s*=)/g, "globalThis.__up_loc$1");
                
                if (js.includes('__up_loc')) {
                    const miniSandbox = `if(!globalThis.__up_loc){globalThis.__up_loc=new Proxy(location,{get(t,p){try{let h=location.href,i=h.indexOf('${proxyOrigin}/http');let u=i!==-1?decodeURIComponent(h.substring(i+${proxyOrigin.length}+1)):h;if(p==='href')return u;return['pathname','origin','host','hostname','port','protocol','search','hash'].includes(p)?new URL(u)[p]:t[p]}catch(e){return t[p]}}});}\n`;
                    js = miniSandbox + js;
                }
            }
            
            const workerHook = `if(typeof WorkerGlobalScope!=='undefined'&&self instanceof WorkerGlobalScope&&!self.__up_hooked){self.__up_hooked=1;const _P="${proxyOrigin}",_T="${target.href}";const _toP=function(u){if(!u)return u;let s=String(u).trim();if(/^(data:|blob:|javascript:|mailto:|tel:|#)/i.test(s))return s;let pu;try{if(s.startsWith(_P+'/http')){pu=s.substring(_P.length+1);if(pu.includes('%3A%2F%2F'))try{pu=decodeURIComponent(pu)}catch(e){}}else if(s.startsWith(_P+'/')&&!s.startsWith(_P+'/http')){pu=_T+s.substring(_P.length);}else if(s===_P){pu=_T;}else{pu=new URL(s,_T).href;}}catch(e){return s}let m;while((m=pu.match(/^(https?:\\/\\/[^\\/?#]+)\\/+(https?:\\/\\/.*)/i))){pu=m[2];}if(pu.startsWith(_P+'/')){pu=_T+pu.substring(_P.length);}else if(pu===_P){pu=_T;}return _P+'/'+pu;};const _iS=self.importScripts;if(_iS)self.importScripts=function(...a){let p=a.map(u=>{try{return _toP(u)}catch(e){return u}});return _iS.apply(self,p)};const _R=self.Request;if(_R)self.Request=function(i,o){let r=i&&typeof i==='object'&&i.url,u=r?i.url:i;try{if(typeof u==='string')u=_toP(u)}catch(e){}return new _R(u,o||(r?i:undefined))};const _f=self.fetch;self.fetch=async function(i,o){let r=i&&typeof i==='object'&&i.url,u=r?i.url:i;try{if(typeof u==='string')u=_toP(u)}catch(e){}if(r){let opt=o?Object.assign({},o):{};['method','headers','mode','credentials','cache','redirect','referrer','integrity','signal'].forEach(k=>{if(i[k]!==undefined)opt[k]=i[k]});if(i.method!=='GET'&&i.method!=='HEAD'&&!opt.body&&!i.bodyUsed){try{opt.body=await i.clone().blob()}catch(e){}}return _f.call(self,u,opt)}return _f.call(self,u,o)};const _W=self.WebSocket;if(_W)self.WebSocket=function(u,p){try{let w=new URL(String(u),location.href);if(w.protocol==='ws:'||w.protocol==='wss:'){if(w.origin!==new URL(_P).origin){let t=(w.protocol==='ws:'?'http:':'https:')+'//'+w.host+w.pathname+w.search;u=_P.replace(/^http/i,'ws')+'/'+t}}}catch(e){}return p?new _W(u,p):new _W(u)};const _o=XMLHttpRequest.prototype.open;XMLHttpRequest.prototype.open=function(m,u,...r){try{u=_toP(u)}catch(e){}return _o.call(this,m,u,...r)};}\n`;
            js = workerHook + js;
        }
    }
...
async function rewriteHTML(res, headers, proxyOrigin, target) {
    ...
    htmlText = htmlText.replace(/<script([^>]*)>([\s\S]*?)<\/script>/gi, (match, attrs, content) => {
        ...
        const isMediaHoster = MEDIA_CDN_REGEX.test(target.hostname);
        if (attrs && /type=['"]?(application\/json|text\/template|text\/x-template|importmap)['"]?/i.test(attrs)) {
            if (isMediaHoster) return match;
            return `<script${attrs}>${rewriteOriginsInText(content, proxyOrigin, target.origin)}</script>`;
        }
        
        if (isMediaHoster || !content.includes('location')) return match;
        
        let rewritten = content.replace(/\b(window|document|top|parent|self)\.location\b/g, "globalThis.__up_loc");
        rewritten = rewritten.replace(/(?<!\w|\.)location\.(pathname|href|origin|host|hostname|port|protocol|assign|replace|search|hash|reload)\b/g, "globalThis.__up_loc.$1").replace(/(?<!\w|\.)location(\s*=)/g, "globalThis.__up_loc$1");
    ...
    return new HTMLRewriter()
        .on("head", { element(el) { 
            if (!injected) { el.prepend(`<base data-up-base="1" href="${proxyOrigin}/${target.origin}/"><meta name="referrer" content="unsafe-url">${payloads.sandbox}`, { html: true }); injected = true; }
        }})
        .on("body", { element(el) { 
            if (!injected) { el.prepend(`<base data-up-base="1" href="${proxyOrigin}/${target.origin}/"><meta name="referrer" content="unsafe-url">${payloads.sandbox}`, { html: true }); injected = true; }
        }})
        .on("base", { element(el) { 
            if (!el.hasAttribute("data-up-base")) {
                safeRewriteAttr(el, "href", proxyOrigin, target);
            }
        }})
        .on("meta", { element(el) {
            let he = (el.getAttribute("http-equiv") || "").toLowerCase();
            let n = (el.getAttribute("name") || "").toLowerCase();
            let p = (el.getAttribute("property") || "").toLowerCase();
            ...
            if (c) {
                if (he === 'refresh') {
                    ...
                    el.setAttribute("content", c.replace(/charset=[^;]+/gi, "charset=utf-8"));
                } else if (n.endsWith('-url') || p.endsWith('-url')) {
                    if (!IGNORE_URL_REGEX.test(c.trim()) && !c.trim().startsWith(proxyOrigin + '/http')) {
                        try { el.setAttribute("content", proxyOrigin + "/" + new URL(c.trim(), target).href); } catch {}
                    }
                }
            }
        ...
        .on("include-fragment, poll-include-fragment, turbo-frame, lazy-partial", { element(el) { 
            safeRewriteAttr(el, "src", proxyOrigin, target); 
            safeRewriteAttr(el, "data-base-src", proxyOrigin, target);
            safeRewriteAttr(el, "data-url", proxyOrigin, target);
        }})
...
function buildUIResponse(title, subtitle, content, status = 200, isHome = false) {
    ...
    const payloadData = { title, subtitle, content, isHome, status, color, homePath };
    const b64Data = btoa(encodeURIComponent(JSON.stringify(payloadData)));
    
    const uiHtml = `<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no"><title>System Dashboard</title><style>${UI_CSS}</style></head><body><div class="box" id="app-mount"><div style="padding:2rem 0;display:flex;justify-content:center"><span class="spinner" style="border-color:#3b82f6;border-bottom-color:transparent"></span></div></div><script>!function(){try{var d=JSON.parse(decodeURIComponent(atob("${b64Data}")));document.title=d.title;var h='<h2 style="color:'+d.color+'">'+d.title+'<span class="badge" style="background:'+d.color+'">'+(d.isHome?'v1.0.0 beta':d.subtitle)+'</span></h2><p class="desc">'+(d.isHome?'简易的无端代理（Web Proxy），支持一些简单站点的浏览。非自部署，严禁进行任何登录动作！':'系统主动干预，当前网络请求已被安全策略拦截。')+'</p>';if(d.isHome){h+='<form id="searchForm"><div class="engine-select"><span>搜索引擎 / 直达 URL</span><select id="engine"><option value="ddg">DuckDuckGo</option><option value="startpage">Startpage</option><option value="github">GitHub</option><option value="yandex">Yandex</option><option value="direct">直接访问</option></select></div><div class="input-group"><input type="text" id="query" placeholder="完整网址或关键词... (按 Enter 出发)" required autocomplete="off" autofocus></div><button type="submit" id="submitBtn"><span>立即前往</span></button></form>'}else{h+='<div class="err-box">'+d.content+'</div>';if(d.status!==401)h+='<button onclick="try{window.top.location.href=window.location.origin+\\''+d.homePath+'\\'}catch(e){window.location.href=window.location.origin+\\''+d.homePath+'\\'}" style="width:100%">返回系统主页</button>'}document.getElementById('app-mount').innerHTML=h;${UI_JS}}catch(e){}}();</script></body></html>`;
    ...
    return new Response(uiHtml, { status, headers: h });
}
